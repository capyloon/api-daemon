...
impl<R: Read + io::Seek> ZipArchive<R> {
    ...
    /// Get a contained file by name without decompressing it if it's deflated
    pub fn by_name_maybe_raw<'a>(
        &'a mut self,
        name: &str,
        raw_if: CompressionMethod,
    ) -> ZipResult<ZipFile<'a>> {
        ...
        self.files
            ...
            .and_then(move |data| {
                ...
                if data.compression_method == raw_if {
                    Ok(ZipFile {
                        crypto_reader: None,
                        reader: ZipFileReader::Raw(limit_reader),
                    ...
                    match make_crypto_reader(
                        ...
                        Ok(Ok(crypto_reader)) => Ok(ZipFile {
                            crypto_reader: Some(crypto_reader),
                            reader: ZipFileReader::NoReader,
                            data: Cow::Borrowed(data),
                        }),
                        Err(e) => Err(e),
                        Ok(Err(_e)) => Err(ZipError::InvalidArchive("InvalidPassword")),
                    }
                }
            })
    }
