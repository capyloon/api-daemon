...
- Go into the `daemon` directory and run `cargo build --release --features=fake-tokens`.
- Create a reverse port mapping to redirect requests from the device to the host: `adb reverse tcp:8081 tcp:8081`.
- Run the daemon with `RUST_LOG=info WS_RUNTIME_TOKEN=secrettoken cargo run --release`.
